...
CREATE TABLE IF NOT EXISTS `MYSHOP22`.`Produttore` (
  ...
  `Nazione` VARCHAR(45) NOT NULL,
  `product_idproduct` INT NOT NULL,
  `Subproduct_idSubproduct` INT NOT NULL,
  PRIMARY KEY (`idProduttore`),
  INDEX `fk_Produttore_product1_idx` (`product_idproduct` ASC) VISIBLE,
  INDEX `fk_Produttore_Subproduct1_idx` (`Subproduct_idSubproduct` ASC) VISIBLE,
  CONSTRAINT `fk_Produttore_product1`
    FOREIGN KEY (`product_idproduct`)
    REFERENCES `MYSHOP22`.`product` (`idproduct`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Produttore_Subproduct1`
    FOREIGN KEY (`Subproduct_idSubproduct`)
    REFERENCES `MYSHOP22`.`Subproduct` (`idSubproduct`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
